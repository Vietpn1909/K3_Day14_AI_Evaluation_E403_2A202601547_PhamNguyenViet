...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"Score the following answer on a scale of 0.0 to 1.0 for each criterion.\n\n"
            f"Question: {question}\n\n"
            f"Answer: {answer}\n\n"
            f"Rubric:\n{rubric_text}\n\n"
            f"Return a JSON object mapping each criterion to its score."
        )
        reasoning = self.judge_llm_fn(prompt)
        import json as _json
        try:
            parsed = _json.loads(reasoning)
            scores = {criterion: float(parsed.get(criterion, 0.5)) for criterion in rubric}
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        regressions = [
            m for m in metrics if new_avgs[m] < baseline_avgs[m] - 0.05
        ]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_metric = min(scores, key=scores.get)  # type: ignore[arg-type]
        ...
        if min_metric == "relevance":
            return "Answer does not address the question — improve prompt clarity"
        if min_metric == "completeness":
    ...
    def generate_improvement_suggestions(
        ...
        if "off_topic" in categories:
            suggestions.append(
                "Enhance intent detection to keep responses on topic"
            )
